...
<div class="row">
    <div class="col-md-4">
        <a href="{{ route('addSizes') }}" class="btn btn-primary">Add sizes</a>
    </div>
...
<div class="container card" style="padding:15px;">
    ...
    <table class="table table-striped table-hover dt-responsive" cellspacing="0" width="100%" id="tableId">
        ...
        @foreach($data as $key=>$row)
            <tr>
                <td>{{$key+1}}</td>
                <td>{{$row->width}}×{{$row->height}} {{$row->unit}}</td>
                @if($row->status==1)
                ...
                @endif
                <td>
                <a href="editSize/{{$row->id}}"  title="edit"><span class="badge badge-warning"><i class="fa fa-edit"></i></span></a>
              
                <a href="javascript:void(0)" onclick="return delete_size(this.id)" id="{{$row->id}}" title="delete"><span class="badge badge-danger"><i class="fa fa-trash"></i></span></a>
                </td>
            </tr>
